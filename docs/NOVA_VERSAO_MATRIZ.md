...
| 9.2 | Importação por PDF (OCR, 1 ou várias pessoas) | 🔴 **Quebrada em produção** — implementada na Etapa 7 (migração `0034`), mas `pdf-parse`/`pdfjs-dist` não roda no runtime serverless da Vercel (`@napi-rs/canvas` não instala, `DOMMatrix is not defined` ao avaliar o módulo). Isolada via `import()` dinâmico pra não derrubar a importação por Excel junto (achado testando em produção), mas a importação por PDF em si segue indisponível até trocar de biblioteca ou resolver a dependência nativa |
