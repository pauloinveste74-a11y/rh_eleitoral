...
**IP/user-agent (Nova versão, Etapa 6 — `0033`)**: trigger
`audit_logs_set_request_metadata` (`before insert`) preenche
`ip_address`/`user_agent` sozinho, lendo `request_headers()` →
`current_setting('request.headers', true)` — GUC que o PostgREST expõe
com os headers HTTP da requisição. Nenhuma das funções acima precisou
mudar; cobre função futura também. `request_headers()`/
`request_ip_address()`/`request_user_agent()` não são chamáveis via RPC
(revogadas de `anon`/`authenticated`) — só uso interno do trigger.
...
| `submit_registration_for_review(p_person_id)` | `authenticated` | Chamador é `administrador`/`rh`, ou é a própria pessoa (`profiles.person_id = p_person_id`); ≥ 1 documento `ativo` | Cria `registration_submissions` (`aguardando_validacao_gestor`), marca `people.status = 'aguardando_gestor'`, resolve correção pendente anterior (`correction_requests.resolved_at`, Etapa 11; grava `new_values` via `snapshot_correction_fields()` desde a Etapa 6). Usada por `/meu-cadastro`. |
| `submit_public_registration_for_review(p_token)` | **`anon`** e `authenticated` | Convite com `person_id` preenchido; ≥ 1 documento `ativo` | Mesmo papel da anterior, variante sem sessão pro cabo eleitoral. **Desde a Etapa 11, não exige mais `invite.status ≠ 'concluido'`** — permite reenvio depois de correção; grava `new_values` desde a Etapa 6. |
...
| Função | Quem chama | Verifica | O que faz |
| --- | --- | --- | --- |
| `decide_registration_submission(p_submission_id, p_decision, p_reason?, p_field_names?)` | `authenticated` | `administrador`/`rh`, ou a pessoa cujo `people.id` bate com `manager_person_id` da submissão (gestor pessoa física); submissão em `aguardando_validacao_gestor` | `aprovar` → `aprovado_gestor`; `rejeitar` → `rejeitado`; `solicitar_correcao` → `correcao_solicitada` + cria `correction_requests` com os campos apontados (`p_field_names`, Etapa 11) e `previous_values` (snapshot via `snapshot_correction_fields()`, Etapa 6). Audita com insert direto (gestor comum não é admin/rh). |
| `decide_rh_validation(p_submission_id, p_decision, p_reason?, p_field_names?)` | `authenticated` | **Só** `administrador`/`rh` (sem caminho "gestor pessoa física" — diferente da anterior); submissão em `aprovado_gestor` | `validar` → `validado`; `rejeitar` → `rejeitado`; `solicitar_correcao` → `correcao_solicitada` + `correction_requests` (com `previous_values` desde a Etapa 6). Audita via `log_audit_event()` direto (só admin/rh chama, sempre pode). |
| `snapshot_correction_fields(p_person_id, p_field_names)` *(Nova versão, Etapa 6 — `0033`)* | uso interno (não exposta a `authenticated`/`anon`) | — | Tira uma foto dos campos apontados (a partir de `people` + os 3 satélites), devolve `jsonb` — usada por `decide_registration_submission`/`decide_rh_validation` (`previous_values`) e `submit_registration_for_review`/`submit_public_registration_for_review` (`new_values`). |
